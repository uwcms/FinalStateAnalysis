...
class MiniAODJetIdEmbedder2016 : public edm::EDProducer {
  public:
    MiniAODJetIdEmbedder2016(const edm::ParameterSet& pset);
    virtual ~MiniAODJetIdEmbedder2016(){}
    void produce(edm::Event& evt, const edm::EventSetup& es);
  private:
    edm::EDGetTokenT<edm::View<pat::Jet> > srcToken_;
};

MiniAODJetIdEmbedder2016::MiniAODJetIdEmbedder2016(const edm::ParameterSet& pset) {
  srcToken_ = consumes<edm::View<pat::Jet> >(pset.getParameter<edm::InputTag>("src"));
  produces<pat::JetCollection>();
}

void MiniAODJetIdEmbedder2016::produce(edm::Event& evt, const edm::EventSetup& es) {
  ...
  for (size_t i = 0; i < input->size(); ++i) {
    ...
    if (std::abs(jet.eta()) > 2.7 && std::abs(jet.eta()) <= 3.0) {
      if (jet.neutralEmEnergyFraction() <= 0.01){
        loose = false;
        tight = false;
      }
      if (jet.neutralHadronEnergyFraction() >= 0.98) {
        tight = false;
        loose = false;
      }
...
DEFINE_FWK_MODULE(MiniAODJetIdEmbedder2016);

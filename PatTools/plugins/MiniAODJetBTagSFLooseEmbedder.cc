...
class MiniAODJetBTagSFLooseEmbedder : public edm::EDProducer {
  public:
    MiniAODJetBTagSFLooseEmbedder(const edm::ParameterSet& pset);

    virtual ~MiniAODJetBTagSFLooseEmbedder(){
        delete calib;
        delete reader_light;
...
MiniAODJetBTagSFLooseEmbedder::MiniAODJetBTagSFLooseEmbedder(const edm::ParameterSet& pset) {
  srcToken_ = consumes<edm::View<pat::Jet> >(pset.getParameter<edm::InputTag>("src"));
  produces<pat::JetCollection>();

  std::string base = std::getenv("CMSSW_BASE");
  std::string fEff =   "/src/FinalStateAnalysis/PatTools/data/htautau_btagging_efficiencies_Loose_76x.root";
...
void MiniAODJetBTagSFLooseEmbedder::produce(edm::Event& evt, const edm::EventSetup& es) {
  ...
  for (size_t i = 0; i < input->size(); ++i) {
      ...
      if (!(pt<20 || std::abs(eta)>2.4)) {
        ...
        if (jet.bDiscriminator("pfCombinedInclusiveSecondaryVertexV2BJetTags")>0.460) pass =true;
        ...
        jet.addUserFloat("btaggedupL", float(btaggedup));
        jet.addUserFloat("btaggeddownL", float(btaggeddown));
        jet.addUserFloat("passL", float(pass));
        jet.addUserFloat("btagSFL", SF);
        jet.addUserFloat("btagSFupL", SFup);
        jet.addUserFloat("btagSFdownL", SFdown);
        jet.addUserFloat("btagEffL", eff);
...
DEFINE_FWK_MODULE(MiniAODJetBTagSFLooseEmbedder);

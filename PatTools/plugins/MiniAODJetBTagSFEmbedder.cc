...
void MiniAODJetBTagSFEmbedder::produce(edm::Event& evt, const edm::EventSetup& es) {
  ...
  for (size_t i = 0; i < input->size(); ++i) {
      ...
      jet.addUserFloat("btagged", float(btagged));
      jet.addUserFloat("btaggedup", float(btaggedup));
      jet.addUserFloat("btaggeddown", float(btaggeddown));
      jet.addUserFloat("pass", float(pass));
      jet.addUserFloat("btagSF", SF);
      jet.addUserFloat("btagSFup", SFup);
      jet.addUserFloat("btagSFdown", SFdown);
      jet.addUserFloat("btagEff", eff);

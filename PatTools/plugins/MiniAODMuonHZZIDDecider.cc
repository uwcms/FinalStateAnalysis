...
class MiniAODMuonHZZIDDecider : public edm::stream::EDProducer<>
{
public:
...
private:
  ...
  bool passVertex(const edm::Ptr<pat::Muon>& mu,
  ...
  // Data
  const edm::EDGetTokenT<edm::View<pat::Muon> > muonCollectionToken_;
...
void MiniAODMuonHZZIDDecider::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(edm::View<pat::Muon>::const_iterator mi = muonsIn->begin();
      mi != muonsIn->end(); mi++) // loop over muons
    {
      ...
      bool idResult = (passKinematics(mptr) && passVertex(mptr, vertices->at(0)) && passType(mptr));
...
bool MiniAODMuonHZZIDDecider::passVertex(const edm::Ptr<pat::Muon>& mu,
                                         const reco::Vertex& vtx) const
{
  return (fabs(mu->dB(pat::Muon::PV3D))/mu->edB(pat::Muon::PV3D) < sipCut && 
	  fabs(mu->muonBestTrack()->dxy(vtx.position())) < pvDXYCut &&
	  fabs(mu->muonBestTrack()->dz(vtx.position())) < pvDZCut);
}

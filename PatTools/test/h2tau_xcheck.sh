...
./patTuple_cfg.py isMC=1 globalTag=$mcgt inputFiles=$gghFile reportEvery=100 \
  outputFile=/scratch/$LOGNAME/h2tau_ggh_xcheck.root dataset=Fall11 &> ggH_tuplization.log &
...
./patTuple_cfg.py isMC=1 globalTag=$mcgt inputFiles=$vbfFile reportEvery=100 \
  outputFile=/scratch/$LOGNAME/h2tau_vbf_xcheck.root dataset=Fall11 &> vbf_tuplization.log &

...
./patTuple_cfg.py isMC=1 globalTag=$mcgt inputFiles=$wjfile reportEvery=100 \
  outputFile=/scratch/$LOGNAME/smb_wjets.$label.root dataset=Fall11 &> wjets_tuplization.log &

./patTuple_cfg.py isMC=1 globalTag=$mcgt inputFiles=$ttbarfile reportEvery=100 \
  outputFile=/scratch/$LOGNAME/smb_ttbar.$label.root dataset=Fall11 &> ttbar_tuplization.log &

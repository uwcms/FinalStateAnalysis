...
void FinalStateMassResolution::init(const edm::EventSetup &iSetup) {
...
getLeaves(const reco::Candidate &c, 
    ...
    if (c.hasMasterClonePtr()) {
        ...
        getLeaves(*c.masterClone(), out);
               ...
               (c.pdgId() != 22 || 
		dynamic_cast<const reco::PFCandidate *>(&c) == 0)) { 
      // but not PF photons:
...
fillP3Covariance(const reco::Candidate &c, 
  ...
  if ((gsf = dynamic_cast<const reco::GsfElectron *>(&c)) != 0) {
    ...
    throw cms::Exception("FinalStateMassResoultion") 
      << "No handle for candidate of type " 

...
def saveplot(filename):
    ...
    canvas.Update()
    canvas.Print(os.path.join("plots", 'eMu3', filename + filetype))
    canvas.SetLogy(True)
    canvas.Update()
    canvas.Print(os.path.join("plots", 'eMu3',
...
selections = {
    ...
    'os_et' :
    'TauCharge*ElecCharge < 0',

    'os_mt' :
    'TauCharge*MuCharge < 0',
...
fr_weight_cmd ="(%s && Mu_MuRelIso > 0.3)*(((0.008051 + 1.603442*exp(-0.100484*(MuPt + Mu_MuRelIso*MuPt))))/(1-(0.008051 + 1.603442*exp(-0.100484*(MuPt + Mu_MuRelIso*MuPt)))))"
jetPt_fr = "(0.192451 + 0.516077*exp(-0.064681*(ElecPt + ElecPt*Elec_ERelIso)))"
jetPt_weight = "(((0.192451 + 0.516077*exp(-0.064681*(ElecPt + ElecPt*Elec_ERelIso))))/(1-(0.192451 + 0.516077*exp(-0.064681*(ElecPt + ElecPt*Elec_ERelIso)))))"
fr_weight_cmd ="(%s && (Elec_EID_WWID < 0.5 || Elec_ERelIso > 0.3))*(" + jetPt_weight + ")"

regions = {
    'qcd_fr_region' : ['kine', 'ss_em', 'anti_mu_iso', 'hps_fail', 'b_veto', 'e_mt'],
    'qcd_final_region' : ['kine', 'ss_em', 'anti_mu_iso', 'hps_fail', 'e_iso', 'b_veto', 'e_mt'],
    'os_fr_region' : ['kine', 'os_em', 'mu_iso', 'hps_pass', 'mu_veto', 'b_veto', 'ht_cut', 'vtx_cut'],
    #'os_fr_region_ttbar' : ['kine', 'os_em', 'e_iso', 'hps_pass', 'ttbar_enriched'],
    'os_fr_region_ttbar' : ['kine', 'os_et', 'mu_iso', 'hps_pass', 'ttbar_enriched'],
    'ss_fr_region' : ['kine', 'ss_em', 'hps_pass', 'mu_iso'],
    'ss_fr_super_region' : ['kine', 'ss_em', 'hps_pass', 'mu_iso', 'mu_veto', 'b_veto', 'ht_cut', 'vtx_cut'],
    'os_final_region' : ['kine', 'os_em', 'mu_iso', 'e_iso', 'hps_pass', 'mu_veto', 'b_veto', 'ht_cut', 'vtx_cut'],
    #'os_final_region_ttbar' : ['kine', 'os_em', 'mu_iso', 'e_iso', 'hps_pass', 'ttbar_enriched'],
    'os_final_region_ttbar' : ['kine', 'os_et', 'mu_iso', 'e_iso', 'hps_pass', 'ttbar_enriched'],
...
variables = [
    ...
    (('MuJetPt', 'MuPt + MuPt*Mu_MuRelIso'), 'Mu Jet p_{t}', [100, 0, 200]),
    ('MuPt', 'Mu p_{t}', [100, 0, 200]),
    (('EJetPt', 'ElecPt + ElecPt*Elec_ERelIso'), 'Electron Jet p_{t}', [100, 0, 200]),
    ('ElecPt', 'Electron p_{t}', [100, 0, 200]),
    #('METPt',  'MET', [100, 0, 200]),
...
tau_pt_denominator = plotter.get_histogram(
    'data_MuEG', '/emt/final/Ntuple:%s' % 'qcd_fr_region_EJetPt',
    rebin = 10,
    show_overflows=True,
)

tau_pt_numerator = plotter.get_histogram(
    'data_MuEG', '/emt/final/Ntuple:%s' % 'qcd_final_region_EJetPt',
    rebin = 10,
            show_overflows=True,
)
...
tau_pt_fake_rate.GetHistogram().GetXaxis().SetTitle("Electron jet p_{T}")
tau_pt_fake_rate.GetHistogram().SetTitle("Electron Isolation Fake Rate vs. E p_{T} in QCD")
tau_pt_fake_rate.GetHistogram().SetMaximum(1.0)

jetPt_fr = "(0.192451 + 0.516077*exp(-0.064681*(x)))"
zmm_pt_fake_rate_func = ROOT.TF1('zmm_fr', jetPt_fr, 0, 200)
zmm_pt_fake_rate_func.SetLineColor(ROOT.EColor.kRed)
zmm_pt_fake_rate_func.Draw('same')
saveplot('qcd_fakerate_elec_pt')

## Compute mu fake rate in ttbar control region
#tau_pt_denominator = plotter.get_histogram(
    #'data_MuEG', '/emt/final/Ntuple:%s' % 'os_fr_region_ttbar_MuPt',
    #rebin = 10,
            #show_overflows=True,
#)

#tau_pt_numerator = plotter.get_histogram(
    #'data_MuEG', '/emt/final/Ntuple:%s' % 'os_final_region_ttbar_MuPt',
    #rebin = 10,
            #show_overflows=True,
#)
...
#tau_pt_fake_rate = ROOT.TGraphAsymmErrors(
    #tau_pt_numerator.th1, tau_pt_denominator.th1)
#tau_pt_fake_rate.Draw("ape")
#tau_pt_fake_rate.GetHistogram().GetXaxis().SetTitle("Mu p_{T}")
#tau_pt_fake_rate.GetHistogram().SetTitle("Tau Isolation Fake Rate vs. mu p_{T} in ttbar")
...
#zmm_pt_fake_rate_func.SetLineColor(ROOT.EColor.kRed)
#zmm_pt_fake_rate_func.Draw('same')
#saveplot('ttbar_fakerate_mu_pt')


## Compute tau jet ptfake rate in ttbar control region
#tau_jetpt_denominator = plotter.get_histogram(
    #'data_MuEG', '/emt/final/Ntuple:%s' % 'os_fr_region_ttbar_MuJetPt',
    #rebin = 10,
            #show_overflows=True,
#)

#tau_jetpt_numerator = plotter.get_histogram(
    #'data_MuEG', '/emt/final/Ntuple:%s' % 'os_final_region_ttbar_MuJetPt',
    #rebin = 10,
            #show_overflows=True,
#)
...
#tau_jetpt_fake_rate.GetHistogram().SetTitle("Mu Isolation Fake Rate vs. jet p_{T} in ttbar")
...
#saveplot('ttbar_fakerate_mu_jetpt')

...
std::vector<double>
PATFinalState::pairMvaMet(size_t i, size_t j, size_t chanNum ) const {
  //std::cout << "pair mets daughters. chan: " << chanNum << std::endl;
  ...
  std::vector<pat::MET> pairMets;
  if (chanNum == 0) {
    pairMets = evt()->pairMvaMetsTT();}
  else if (chanNum == 1) {
    pairMets = evt()->pairMvaMetsEM();}
  if (pairMets.size() == 0) return failedV;
  else {
